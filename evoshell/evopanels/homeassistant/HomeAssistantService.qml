...
Item {
    ...
    function traySettings() {
        if (shell && shell.shellConfig && shell.shellConfig.homeAssistant)
            return shell.shellConfig.homeAssistant
        return ({})
    }
    ...
    readonly property int refreshIntervalSec: Theme.pollHomeAssistantSec
    ...
    function lightHasBrightness(entityId) {
        if (!Array.isArray(root.lightBrightnessEntities) || root.lightBrightnessEntities.length === 0)
            return true
        return root.entityAllowed(root.lightBrightnessEntities, entityId)
    }

    function lightHasColor(entityId) {
        if (!Array.isArray(root.lightColorEntities) || root.lightColorEntities.length === 0)
            return true
    ...
    function flushClimateModeRequest() {
        ...
        climateProc.command = Api.climateModeCommand(root.home, root.evoshellBin, entityId, mode)
    ...
    function flushClimateTempRequest() {
        ...
        climateProc.command = Api.climateTempCommand(root.home, root.evoshellBin, entityId, temperature)
        climateProc.running = true
    }
    ...
    function refresh(fresh) {
        ...
        statesProc.command = Api.statesCommand(root.home, root.evoshellBin, root.configJson(), fresh === true)
    ...
    function setLightBrightness(entityId, brightnessPct) {
        ...
        lightQueue.command = Api.lightCommand(root.home, root.evoshellBin, id, "on", pct)
    ...
    function setLight(entityId, on, brightnessPct) {
        ...
        lightQueue.command = on
            ? Api.lightCommand(root.home, root.evoshellBin, id, "on", brightnessPct)
            : Api.lightCommand(root.home, root.evoshellBin, id, "off")
        lightDebounce.restart()
    }
    ...
    function setLightHue(entityId, hue, red, green, blue, brightnessPct, saturationPct) {
        if (!entityId)
            return
        var id = String(entityId)
        var rgb = [
        ...
        var hsS = Math.max(0, Math.min(100, Math.round(Number(saturationPct))))
        if (isNaN(hsS))
            hsS = 100
        var hs = [hsH, hsS]
        ...
        lightDebounce.stop()
        lightQueue.entityId = id
        lightQueue.command = Api.lightHueCommand(root.home, root.evoshellBin, id, h, pct, hsS)
    ...
    function openDashboard() {
        ...
        Quickshell.execDetached([
            ...
            "open", "brave-home-assistant", root.haUrl
        ])
    }
