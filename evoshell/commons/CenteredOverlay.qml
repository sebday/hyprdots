...
Item {
    ...
    readonly property int resolvedContentHeight: {
        ...
        if (target && target.implicitHeight > 0)
            measured = target.implicitHeight + verticalInset
        else
            measured = Math.max(contentHost.childrenRect.height + verticalInset, 1)
        if (maxContentHeight > 0)
            return Math.min(measured, maxContentHeight)
        return measured
    }
