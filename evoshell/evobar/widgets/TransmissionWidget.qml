...
Item {
    ...
    readonly property string hoverPanelId: settings.onHover
        ...
        : (trayMode ? "evo.panels.network.transmission" : "")
    ...
    readonly property string script: Util.evoshellScript(home, shell, "evo-bar-transmission-bar")
    ...
    MouseArea {
        ...
        onClicked: function(mouse) {
            if (mouse.button === Qt.LeftButton && root.shell) {
                ...
                root.shell.summon("evo.panels.network.stats", '{"transmissionAdd":true}')
                root.shell.pinHoverPanel("evo.panels.network.stats")
                return
            }
