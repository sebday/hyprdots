...
Item {
    ...
    readonly property string cacheKey: "evo.panels.network.stats"
    readonly property string script: Util.evoshellScript(Quickshell.env("HOME"), shell, "evo-bar-transmission popup")
    readonly property int hintFont: Theme.fontSizeL
    readonly property int titleFont: Theme.fontSize2xl
    readonly property int statFont: Theme.fontSizeXl
    readonly property string transmissionBin: Util.evoshellScript(Quickshell.env("HOME"), shell, "evo-bar-transmission")
    ...
    function publishCache(json) {
        if (!shell || !json)
            return
        var existing = Util.hoverPanelCacheRead(shell, cacheKey)
        var next = {}
        ...
        Util.hoverPanelCacheWrite(shell, cacheKey, next)
    }
