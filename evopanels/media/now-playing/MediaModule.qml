...
Item {
    ...
    readonly property int nowPlayingTitleFont: Theme.fontSizeL
    ...
    readonly property string cacheKey: "evo.panels.media.now-playing"
    ...
    readonly property int libraryTileSpacing: Theme.hoverPanelSectionSpacing
    ...
    readonly property var evoPlayerMonitor: shell ? shell.serviceFor("evo.panels.player.monitor") : null
    ...
    readonly property int posterGridWidth: Math.max(0, hoverPanelWidth - Theme.hoverPanelContentPad * 2)
    ...
    function publishCache() {
        if (!shell)
            return
        Util.hoverPanelCacheWrite(shell, cacheKey, {
            films: mediaFilms,
            shows: mediaShows
        })
    }
    ...
    function openMediaLibrary(showName, tab) {
        ...
        shell.summon("evo.panels.media.library", JSON.stringify(payload))
    }
    ...
    function toggleEvoPlayback() {
        Quickshell.execDetached(Util.evoCommand(home, ["player", "toggle"]))
    }

    function evoPlayerPrevious() {
        Quickshell.execDetached(Util.evoCommand(home, ["player", "prev"]))
    }

    function evoPlayerNext() {
        Quickshell.execDetached(Util.evoCommand(home, ["player", "next"]))
    }
    ...
    function stopEvoplayerPlayback() {
        Quickshell.execDetached(Util.evoCommand(home, ["player", "stop"]))
    }
    ...
    ColumnLayout {
        ...
        component PlayingFeedPanel: SectionPanel {
            ...
            readonly property bool feedCanStop: isMpris
                ...
                : true
            readonly property string feedLegendLabel: {
                if (isMpris && mprisPlayer)
                    return root.mprisPlaybackLabel(mprisPlayer)
                if (isEvo)
                    return root.evoPlayerPlaying
                        ? "Playing"
                        : (root.evoPlayerPaused ? "Paused" : "Stopped")
            ...
            HoverPanelLabelPill {
                text: feedPanel.feedLegendLabel
                icon: "󰎈"
                fontSize: Theme.fontSizeS
            }
