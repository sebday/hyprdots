...
Item {
    ...
    readonly property var audio: shell ? shell.serviceFor("evo.bar.media.audio") : null
    ...
    function openMediaLibrary() {
        ...
        if (shell) {
            shell.toggle("evo.bar.media.library", "")
            return
        }
        Quickshell.execDetached(["bash", (Quickshell.env("HOME") || "") + "/.local/bin/evo-ipc", "shell", "toggle", "evo.bar.media.library"])
    }
