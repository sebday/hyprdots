// ==UserScript==
// @name        Omarchy Theme Hot-Reloader
...
// @grant       GM_setValue
// @version     1.0.1
// @updateURL   http://localhost:8008/shared/orangemonkey-theme-reloader.js
// @downloadURL http://localhost:8008/shared/orangemonkey-theme-reloader.js
// @author      Seb Day
// @description Hot-reloads Omarchy theme CSS for multiple sites via darkhttpd.
// ==/UserScript==
...
(function () {
	...
	const STYLE_ID = `omarchy-hot-reload-style-${host}`;
	const CACHE_KEY = `omarchy-theme-css-v1-${host}`;
	const siteCssUrl =
		siteCss === "shoelace-hex.css"
	...
	function checkForUpdate() {
		Promise.all(themeUrls.map(fetchCSS))
			.then((parts) => {
				...
				console.log(`Omarchy theme updated for ${host}.`);
			})
			.catch(() => {});
	}
