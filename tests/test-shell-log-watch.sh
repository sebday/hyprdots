...
icon_msg='WARN: Could not load icon "/tmp/evo-screenshot.png?rev=123" at size QSize(102, 102) from request'
icon_json="$("$watch" format "$icon_msg")"
assert_eq "icon load title" "icon load failed" "$(jq -r '.title' <<< "$icon_json")"
assert_eq "icon load body" "evo-screenshot.png" "$(jq -r '.body' <<< "$icon_json")"
