...
Item {
    ...
    readonly property string hoverPanelId: settings.onHover ? String(settings.onHover) : "evo.panels.calendar"
    ...
    HoverHandler {
        enabled: root.hoverPanelId !== ""
        onHoveredChanged: {
            if (!root.shell || !root.hoverPanelId) return
            if (hovered)
                root.shell.hoverEnter(root.hoverPanelId, root, root.barPanel)
            else
                root.shell.hoverLeave(root.hoverPanelId)
        }
    }
