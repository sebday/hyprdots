...
Item {
    ...
    readonly property string volumeHoverPanelId: settings.onHover
    ...
    readonly property var audio: shell ? shell.serviceFor("evo.sys.media.audio") : null
    ...
    function openMixer() {
        Util.dismissHoverPanelFromBar(shell, volumeHoverPanelId)
        if (settings.onClickRight)
            Quickshell.execDetached(["bash", "-lc", String(settings.onClickRight)])
        else
            Util.execDetachedTui(["ghostty", "--class=TUI.main", "-e", "wiremix"])
    }
    ...
    function openAlsamixer() {
        ...
        if (settings.onClick)
            ...
            Util.execDetachedTui(["ghostty", "--class=TUI.main", "-e", "alsamixer"])
    }

    function openMediaLibrary() {
        Util.dismissHoverPanelFromBar(shell, mediaHoverPanelId)
        if (shell) {
    ...
    function handleCavaClick(mouse) {
        if (mouse.button === Qt.RightButton) {
            if (Util.pinHoverPanelFromBarIfActive(shell, volumeHoverPanelId))
                return
            openMixer()
            return
        }
    ...
    function setMediaHoverPanel(active) {
        ...
        if (active)
            shell.hoverEnter(mediaHoverPanelId, mediaLabel, barPanel)
        else
    ...
    function setVolumeHoverPanel(active) {
        volumeHovered = active
        if (!shell || !volumeHoverPanelId) return
        if (active)
            shell.hoverEnter(volumeHoverPanelId, volumeAnchorItem(), barPanel)
        else
            shell.hoverLeave(volumeHoverPanelId)
    }
    ...
    function peekVolumePopup() {
        if (!shell || !volumeHoverPanelId || volumeHovered) return
        shell.peekHoverPanel(volumeHoverPanelId, volumeAnchorItem(), barPanel, volumePeekMs)
    }
    ...
    Row {
        ...
        Text {
            ...
            MouseArea {
                ...
                acceptedButtons: Qt.LeftButton | Qt.RightButton
                cursorShape: Qt.PointingHandCursor
                onContainsMouseChanged: root.setVolumeHoverPanel(containsMouse)
                onWheel: function(wheel) { root.handleWheel(wheel) }
