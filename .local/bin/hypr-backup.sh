#!/bin/bash
# Backup project .env files and waybar secrets, preserving paths.
...
WAYBAR_SECRETS="${HOME}/.config/waybar/secrets.env"
...
extra=0
if [ -f "$WAYBAR_SECRETS" ]; then
  extra=1
fi

if [ "${#rel_files[@]}" -eq 0 ] && [ "$extra" -eq 0 ]; then
  echo "no .env files found in $PROJECTS and missing $WAYBAR_SECRETS" >&2
  exit 1
fi
...
if [ "$extra" -eq 1 ]; then
  # Keep .config/waybar/… path inside the archive.
  (cd "$HOME" && zip -q "$OUT" ".config/waybar/secrets.env")
fi

count=$((${#rel_files[@]} + extra))

echo "backed up $count file(s) to $OUT"
notify-send "Hypr backup" "$count file(s) → $(basename "$OUT")"
