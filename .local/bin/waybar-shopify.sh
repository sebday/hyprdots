...
#   WAYBAR_SHOPIFY_TZ       — timezone for "today" (default Europe/London)
...
main() {
    ...
    local tz_name="${WAYBAR_SHOPIFY_TZ:-$DEFAULT_TZ}"
    tz_name="${tz_name// /}"
    ...
    if ! sqlite3 -readonly "$db_path" "SELECT 1;" >/dev/null 2>&1; then
        echo "waybar-shopify: ${db_path}: db error" >&2
    ...
    for i in $(seq 13 -1 0); do
        ...
        chart_sales+=("$(get_day_stats "$db_path" "$day_iso" "$spend_col" | cut -d'|' -f1)")
    done
    ...
    output_text="${STORE_DISPLAY}${sym}${today_sales_val} | ${orders} | ${cos_str} ${sales_chart}"
