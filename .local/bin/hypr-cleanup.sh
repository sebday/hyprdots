...
# Walker / elephant clipboard history
if command -v elephant >/dev/null 2>&1; then
  elephant activate 'clipboard;;remove_all;;' 2>/dev/null || true
  cleared+=("walker clipboard")
fi

# cliphist (backing store for walker clipboard)
