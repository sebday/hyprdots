#!/bin/bash
# Waybar: SPCX USD price + Trading 212 unrealized P/L %.
#
# Credentials: ~/.config/waybar/secrets.env (T212_API_KEY, T212_API_SECRET)
...
if [[ -z "${T212_API_KEY:-}" || -z "${T212_API_SECRET:-}" ]]; then
    json_out "SPCX —" "Set T212_API_KEY and T212_API_SECRET in ~/.config/waybar/secrets.env"
    exit 0
fi
