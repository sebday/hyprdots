...
Item {
    ...
    function pin() {
        ...
        if (shell && effectivePluginId)
            shell.pinHoverPanel(effectivePluginId)
    }
    ...
    function unpin() {
        ...
        if (shell && effectivePluginId)
            shell.unpinHoverPanel(effectivePluginId)
        close()
    }
    ...
    Timer {
        id: revealMaxTimer
        interval: Theme.hoverPanelRevealMaxWait
        repeat: false
        onTriggered: {
            if (root.opened && !root.revealed)
                root.revealed = true
